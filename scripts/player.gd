...
func _on_sprite_animation_finished() -> void:
	# ponytail: only care about crouch reverse finishing (standup done)
...
func _play_anim(anim: StringName) -> void:
	# ponytail: block interrupts while standup reverse is playing
	if sprite.speed_scale < 0.0: return
	if sprite.animation == anim and sprite.is_playing(): return
	sprite.speed_scale = 1.0
	sprite.play(anim)
	# ponytail: Edison frames are uniform 128x204, no adjustment needed
	var karakter = Global.p2_pilihan if is_player_2 else Global.p1_pilihan
	if karakter == "tesla":
		# Walk frame height is 443px. Idle/jump/aw/crouch height is 534px.
		# Scaling 534px down by 0.83 gives 443px, making them the same size!
		# When scaled, their bottoms are naturally aligned on the ground.
		if anim in ["idle", "jump", "aw", "crouch"]:
			_base_scale = 0.83
		else:
			_base_scale = 1.0
		sprite.offset.y = 0.0
	...
	if is_stunned:
		...
		if Input.is_action_pressed(input_crouch) and is_on_floor():
			...
			if sprite.sprite_frames.has_animation("crouch"):
				if not is_crouching:
					# First press: play forward once, AnimatedSprite2D freezes at last frame
					is_crouching = true
					sprite.sprite_frames.set_animation_loop("crouch", false)
					sprite.speed_scale = 1.0
					_play_anim("crouch")
				# else: already crouching, frozen at last frame — do nothing
			else:
				_play_anim("idle")
		elif is_crouching and is_on_floor():
			# Released: play backward from last frame to standup
			is_crouching = false
			sprite.speed_scale = -1.0
			sprite.play("crouch")
			sprite.frame = sprite.sprite_frames.get_frame_count("crouch") - 1
			# _on_sprite_animation_finished resets speed_scale and plays idle
			var karakter = Global.p2_pilihan if is_player_2 else Global.p1_pilihan
			if karakter == "tesla":
				_base_scale = 0.83
				sprite.offset.y = 0.0
			...
			if direction:
				...
				# ponytail: only play walk on floor so it doesn't override air walk (aw)
				if is_on_floor():
					_play_anim("walk")
